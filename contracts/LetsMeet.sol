...
contract LetsMeet {
    ...
    function newProposal(string memory _what, string memory _when) public returns (uint256 proposalId) {
    ...
    function counterProposal(uint256 proposalId, string memory _what, string memory _when) public returns (uint256 counterId) {
    ...
    function getScore(uint256 proposalId) public returns (uint score) {
        LetsMeetProposal storage proposal = proposals[proposalId-1];
        return proposal.yayVotes - proposal.nayVotes - proposal.counterIds.length;
    }

    function getNumCounters(uint256 proposalId) public returns (uint256 numCounters) {
